...
class DefectDetector:
    ...
    def detect_defects(self, test_cases: List[Tuple], verbose: bool = True) -> Tuple[List[Tuple], int]:
        ...
        for x, x_prime, label, relation_type, test_name in test_cases_iter:
            ...
            x_inputs = {k: v for k, v in x.items() if k != 'token_type_ids'}
            x_prime_inputs = {k: v for k, v in x_prime.items() if k != 'token_type_ids'}

            x_logits = self.model.predict(x_inputs)
            x_prime_logits = self.model.predict(x_prime_inputs)
